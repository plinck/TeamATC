...
class Results extends React.Component {
  ...
  componentDidMount() {
    ...
    filterByString =
      this.props.location && this.props.location.state
        ? this.props.location.state.filterByString
        : undefined;
  ...
  displayFilter(results) {
    if (this.state.typeFilter === "All") return results;
    if (this.state.typeFilter === "Swim")
      return results.filter((act) => act.activityType === "Swim");
    if (this.state.typeFilter === "Bike")
      return results.filter((act) => act.activityType === "Bike");
    if (this.state.typeFilter === "Run")
      return results.filter((act) => act.activityType === "Run");
    if (this.state.typeFilter === "Other")
      return results.filter((act) => act.activityType === "Other");
  }

  // ************************************************************
  // Database Interaction Methods
  // ************************************************************
  // Get results with a filter
  getFilteredActivities = (filterByString, lastActivityDoc) => {
    ...
    ActivityDB.getFiltered(filterObj, undefined, lastActivityDoc)
      .then((res) => {
        ...
        this.setState({
          results: results,
          lastActivityDoc: res.lastActivityDoc,
          isLoading: false,
        });
      })
      .catch((err) => {
  ...
  getNextPage = () => {
    ...
    ActivityDB.getFiltered(filterObj, undefined, lastActivityDoc)
      .then((res) => {
        let newActivities = res.results;
        let joined = this.state.results.concat(newActivities);
        this.setState({
          results: joined,
          lastActivityDoc: res.lastActivityDoc,
          isLoading: false,
          ...
          {this.props.user.isAdmin ? (
            <Grid item xs={12} md={4}>
              <ActivityUserSearch
                ...
                searchForActivities={this.searchForActivities}
              />
            </Grid>
          ) : null}
            ...
            <ButtonGroup color="primary">
              <Tooltip title="All">
                <Button
                  ...
                  variant={
                    this.state.typeFilter === "All" ? "contained" : "outlined"
                  }
                >
                  All
                </Button>
              ...
              <Tooltip title="Swim">
                <Button
                  onClick={() => this.filterByType("Swim")}
                  variant={
                    this.state.typeFilter === "Swim" ? "contained" : "outlined"
                  }
                >
                  <PoolIcon />
                </Button>
              ...
              <Tooltip title="Bike">
                <Button
                  onClick={() => this.filterByType("Bike")}
                  variant={
                    this.state.typeFilter === "Bike" ? "contained" : "outlined"
                  }
                >
                  <DirectionsBikeIcon />
                </Button>
              ...
              <Tooltip title="Run">
                <Button
                  onClick={() => this.filterByType("Run")}
                  variant={
                    this.state.typeFilter === "Run" ? "contained" : "outlined"
                  }
                >
                  <DirectionsRunIcon />
                </Button>
              ...
              <Tooltip title="Other">
                <Button
                  onClick={() => this.filterByType("Other")}
                  variant={
                    this.state.typeFilter === "Other" ? "contained" : "outlined"
                  }
                >
                  <BuildIcon />
                </Button>
      ...
      let activityView = (
        <div>
          <div className={classes.act}>
            {this.displayFilter(results).map((activity, index) => {
              return (
                <div key={activity.id}>
                  <Activity
                    activity={activity}
